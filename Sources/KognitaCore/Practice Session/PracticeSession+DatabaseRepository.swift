//
//  PracticeSession+DatabaseRepository.swift
//  KognitaCore
//
...
extension PracticeSession {
    /// A database implementation a `PracticeSessionRepository`
    public struct DatabaseRepository: DatabaseConnectableRepository {
        ...
        init(database: Database, repository: DatabaseRepository) {
            self.database = database
            self.typingTaskRepository = repository.typingTaskRepository
        ...
        private let typingTaskRepository: TypingTaskRepository

        /// The multiple choice repository to use
        private let multipleChoiceRepository: MultipleChoiseTaskRepository

        /// The subject repository to use
        ...
        /// The task result repository to use
...
extension PracticeSession.DatabaseRepository {
            ...
            .flatMap {
                ...
                self.update(submit, in: sessionID, userID: user.id)
                    .failableFlatMap { actionTaken in
                        ...
                        return try self.typingTaskRepository
    ...
    public func extend(session sessionID: PracticeSession.ID, for user: User) throws -> EventLoopFuture<Void> {
        return TaskSession.query(on: database)
            .filter(\TaskSession.$user.$id == user.id)
            .filter(\.$id == sessionID)
            .join(superclass: PracticeSession.DatabaseModel.self, with: TaskSession.self)
            .first(PracticeSession.DatabaseModel.self)
            .unwrap(or: Abort(.badRequest))
            .flatMap { session in
